...
public class GenC : GenCCpp
{
	...
	void WriteMemberAccess(CiExpr left, CiClass symbolClass)
	{
		if (left.Type is CiStorageType)
			...
			Write("->");
		for (CiClass klass = left.Type.AsClassType().Class; klass != symbolClass; klass = (CiClass) klass.Parent)
			Write("base.");
	}
	...
	protected void WriteCCall(CiExpr obj, CiMethod method, List<CiExpr> args)
	{
		...
		if (IsReferenceTo(obj, CiId.BasePtr)) {
			...
			case CiCallType.Virtual:
				if (obj != null)
					klass = obj.Type as CiClass ?? obj.Type.AsClassType().Class;
	...
	protected override void WriteCallExpr(CiExpr obj, CiMethod method, List<CiExpr> args, CiPriority parent)
	{
		...
		case CiId.DictionaryAdd:
			...
			CiStorageType valueType = (CiStorageType) obj.Type.AsClassType().GetValueType();
