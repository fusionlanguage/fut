...
public class CiLexer
{
	...
	static readonly string[] TokenStrings = {
		"end-of-file",
		"identifier",
		"integer constant",
	...
	static string ToString(CiToken token) => TokenStrings[(int) token];

	protected bool Check(CiToken expected)
	{
		if (See(expected))
			return true;
		ReportError($"Expected {ToString(expected)}, got {ToString(this.CurrentToken)}");
		return false;
	}
