...
export const FuId = {
	...
	STRING_CLASS : 21,
	STRING_PTR_TYPE : 22,
	STRING_STORAGE_TYPE : 23,
	MAIN_ARGS_TYPE : 24,
	ARRAY_PTR_CLASS : 25,
	ARRAY_STORAGE_CLASS : 26,
	EXCEPTION_CLASS : 27,
	LIST_CLASS : 28,
	QUEUE_CLASS : 29,
	STACK_CLASS : 30,
	PRIORITY_QUEUE_CLASS : 31,
	HASH_SET_CLASS : 32,
	SORTED_SET_CLASS : 33,
	DICTIONARY_CLASS : 34,
	SORTED_DICTIONARY_CLASS : 35,
	ORDERED_DICTIONARY_CLASS : 36,
	TEXT_WRITER_CLASS : 37,
	STRING_WRITER_CLASS : 38,
	REGEX_OPTIONS_ENUM : 39,
	REGEX_CLASS : 40,
	MATCH_CLASS : 41,
	JSON_ELEMENT_CLASS : 42,
	LOCK_CLASS : 43,
	STRING_LENGTH : 44,
	ARRAY_LENGTH : 45,
	CONSOLE_ERROR : 46,
...
export class FuSystem extends FuScope
{
	constructor()
	{
		...
		this.nIntType.add(FuMethod.new(null, FuVisibility.PUBLIC, FuCallType.NORMAL, this.boolType, FuId.N_INT_TRY_PARSE, "TryParse", true, FuVar.new(this.stringPtrType, "value"), FuVar.new(this.intType, "radix", this.newLiteralLong(0n))));
		...
		let priorityQueueClass = this.#addCollection(FuId.PRIORITY_QUEUE_CLASS, "PriorityQueue", 2, FuId.PRIORITY_QUEUE_CLEAR, FuId.PRIORITY_QUEUE_COUNT);
		priorityQueueClass.addMethod(this.#typeParam0, FuId.PRIORITY_QUEUE_DEQUEUE, "Dequeue", true);
		priorityQueueClass.addMethod(this.voidType, FuId.PRIORITY_QUEUE_ENQUEUE, "Enqueue", true, FuVar.new(this.#typeParam0, "element"), FuVar.new(this.#typeParam1, "priority"));
		...
		mathClass.addStaticMethod(numericType, FuId.MATH_ABS, "Abs", FuVar.new(this.doubleType, "a"));
		...
		mathClass.addStaticMethod(numericType, FuId.MATH_CLAMP, "Clamp", FuVar.new(this.doubleType, "value"), FuVar.new(this.doubleType, "min"), FuVar.new(this.doubleType, "max"));
		...
		mathClass.addStaticMethod(numericType, FuId.MATH_MAX, "Max", FuVar.new(this.doubleType, "a"), FuVar.new(this.doubleType, "b"));
		mathClass.addStaticMethod(numericType, FuId.MATH_MIN, "Min", FuVar.new(this.doubleType, "a"), FuVar.new(this.doubleType, "b"));
	...
	#typeParam1 = Object.assign(new FuType(), { id: FuId.TYPE_PARAM1, name: "T" });
	...
	newLiteralLong(value, loc = 0)
	{
		...
		return Object.assign(new FuLiteralLong(), { loc: loc, type: type, value: value });
	}

	newLiteralString(value, loc = 0)
	{
		return Object.assign(new FuLiteralString(), { loc: loc, type: this.stringPtrType, value: value });
	}
...
export class FuProgram extends FuScope
{
	...
	{
		let l = 0;
		let r = this.sourceFiles.length - 1;
		while (l < r) {
			let m = (l + r + 1) >> 1;
...
export class FuParser extends FuLexer
{
	...
	#parsePrimaryExpr(type = false, new_ = false)
	{
		let result;
		switch (this.currentToken) {
		case FuToken.INCREMENT:
		case FuToken.DECREMENT:
			...
			return Object.assign(new FuPrefixExpr(), { loc: this.tokenLoc, op: this.nextToken(), inner: this.#parsePrimaryExpr() });
		case FuToken.MINUS:
		case FuToken.TILDE:
		case FuToken.EXCLAMATION_MARK:
			return Object.assign(new FuPrefixExpr(), { loc: this.tokenLoc, op: this.nextToken(), inner: this.#parsePrimaryExpr() });
		case FuToken.NEW:
			...
			result = this.#parsePrimaryExpr(true, true);
	...
	#parseMulExpr()
	{
		...
		for (;;) {
			...
			case FuToken.SLASH:
			case FuToken.MOD:
				left = Object.assign(new FuBinaryExpr(), { loc: this.tokenLoc, left: left, op: this.nextToken(), right: this.#parsePrimaryExpr() });
				break;
			default:
				return left;
			}
	...
	#parseRelExpr()
	{
		...
		for (;;) {
			...
			case FuToken.GREATER_OR_EQUAL:
				...
				break;
			case FuToken.IS:
				let isExpr = Object.assign(new FuBinaryExpr(), { loc: this.tokenLoc, left: left, op: this.nextToken(), right: this.#parsePrimaryExpr() });
				if (this.see(FuToken.ID)) {
...
export class FuConsoleHost extends GenHost
{
	...
	static usage(app)
	{
		...
		console.log("Options:");
		console.log("-o FILE    Write to the specified file, extension determines language");
		...
		console.log("-r FILE.fu Read the specified source file but don't emit code for it");
		console.log("-I DIR     Add directory to resource search path");
		console.log("-l LANG    Set target language explicitly:");
		console.log("           c, cpp, cs, d, java, js, py, swift, ts, d.ts, cl");
...
export class FuSema
{
	...
	#visitInterpolatedString(expr)
	{
		...
		for (let partsIndex = 0; partsIndex < expr.parts.length; partsIndex++) {
			...
			if (this.#coerce(arg, this.#host.program.system.printableType)) {
	...
	#getCurrentContainer()
	{
		for (let scope = this.#currentScope; scope != null; scope = scope.parent) {
			let container;
	...
	#visitSymbolReference(expr)
	{
		...
		if (result != expr)
			return result;
		return Object.assign(new FuSymbolReference(), { loc: expr.loc, left: left, name: expr.name, symbol: expr.symbol, type: expr.type });
	}
	...
	{
		let type = this.#host.program.system.promoteIntegerTypes(left.type, right.type);
	...
	{
		let type = this.#host.program.system.promoteNumericTypes(left.type, right.type);
		this.#coerce(left, type);
		this.#coerce(right, type);
		return type;
	}
	...
	{
		return Object.assign(new FuLiteralDouble(), { loc: expr.loc, type: this.#host.program.system.doubleType, value: value });
	}
		...
		for (;;) {
			...
			if (expr instanceof FuSymbolReference) {
				...
				if (symbol.symbol instanceof FuVar) {
					...
					def.isAssigned = true;
					{
						const fuSwitchValue = symbol.symbol.parent;
						if (fuSwitchValue instanceof FuFor) {
							const forLoop = symbol.symbol.parent;
							forLoop.isRange = false;
						}
						else if (fuSwitchValue instanceof FuForeach)
					...
					{
						const fuSwitchValue = symbol.left.type;
						if (fuSwitchValue instanceof FuStorageType)
							expr = symbol.left;
						else if (fuSwitchValue instanceof FuReadWriteClassType)
							return;
						else if (fuSwitchValue instanceof FuClassType) {
				...
				{
					const fuSwitchValue = indexing.left.type;
					if (fuSwitchValue instanceof FuStorageType)
						expr = indexing.left;
					else if (fuSwitchValue instanceof FuReadWriteClassType)
						return;
	...
	{
		let result = Object.assign(new FuInterpolatedString(), { loc: left.loc, type: this.#host.program.system.stringStorageType });
	...
	#checkComparison(left, right)
	{
		if (left.type instanceof FuEnum)
			...
			let doubleType = this.#host.program.system.doubleType;
	...
	#resolveNew(expr)
	{
		...
		if (type instanceof FuArrayStorageType) {
			const array = type;
			expr.type = Object.assign(new FuDynamicPtrType(), { loc: expr.loc, class: this.#host.program.system.arrayPtrClass, typeArg0: array.getElementType() });
			expr.inner = array.lengthExpr;
			return expr;
		}
		else if (type instanceof FuStorageType) {
			const klass = type;
			expr.type = Object.assign(new FuDynamicPtrType(), { loc: expr.loc, class: klass.class, typeArg0: klass.typeArg0, typeArg1: klass.typeArg1 });
			expr.inner = null;
			return expr;
		}
		else
	...
	#visitPrefixExpr(expr)
	{
		...
		case FuToken.TILDE:
			...
			if (inner.type instanceof FuEnumFlags)
				...
				this.#coerce(inner, this.#host.program.system.intType);
				let notRange;
		...
		return Object.assign(new FuPrefixExpr(), { loc: expr.loc, op: expr.op, inner: inner, type: type });
	}
	...
	#visitPostfixExpr(expr)
	{
		...
		case FuToken.DECREMENT:
			...
			this.#coerce(expr.inner, this.#host.program.system.doubleType);
	...
	#visitBinaryExpr(expr)
	{
		...
		case FuToken.GREATER_OR_EQUAL:
			...
			break;
		case FuToken.COND_AND:
			this.#coerce(left, this.#host.program.system.boolType);
			this.#coerce(right, this.#host.program.system.boolType);
		...
		case FuToken.SHIFT_RIGHT_ASSIGN:
			this.#checkLValue(left);
			this.#coerce(left, this.#host.program.system.intType);
			this.#coerce(right, this.#host.program.system.intType);
			expr.left = left;
			expr.right = right;
			expr.type = left.type;
		...
		return Object.assign(new FuBinaryExpr(), { loc: expr.loc, left: left, op: expr.op, right: right, type: type });
	}
	...
	#tryGetPtr(type, nullable)
	{
		...
		if ((storage = type) instanceof FuStorageType)
			return Object.assign(new FuReadWriteClassType(), { class: storage.class.id == FuId.ARRAY_STORAGE_CLASS ? this.#host.program.system.arrayPtrClass : storage.class, nullable: nullable, typeArg0: storage.typeArg0, typeArg1: storage.typeArg1 });
		let ptr;
	...
	#visitSelectExpr(expr)
	{
		...
		if (cond instanceof FuLiteralFalse)
			return onFalse;
		return Object.assign(new FuSelectExpr(), { loc: expr.loc, cond: cond, onTrue: onTrue, onFalse: onFalse, type: type });
	}

	#evalType(generic, type)
	{
		switch (type.id) {
		case FuId.TYPE_PARAM0:
			return generic.typeArg0;
		case FuId.TYPE_PARAM0_NOT_FINAL:
			return generic.typeArg0.isFinal() ? null : generic.typeArg0;
		case FuId.TYPE_PARAM1:
	...
	#resolveBool(expr)
	{
		...
		this.#coerce(expr, this.#host.program.system.boolType);
		return expr;
	}
		...
		for (;;) {
			...
			if ((binary = expr) instanceof FuBinaryExpr && binary.op == FuToken.LEFT_BRACKET) {
				if (binary.right != null) {
					...
					let arrayStorage = Object.assign(new FuArrayStorageType(), { class: this.#host.program.system.arrayStorageClass, typeArg0: outerArray, lengthExpr: lengthExpr, length: 0 });
					if (this.#coerce(lengthExpr, this.#host.program.system.intType) && (!dynamic || binary.left.isIndexing())) {
						let literal;
					...
					outerArray = FuSema.#createClassPtr(this.#host.program.system.arrayPtrClass, ptrModifier, nullable);
	...
	#visitLock(statement)
	{
		...
		this.#coerce(statement.lock, this.#host.program.system.lockPtrType);
		this.#visitStatement(statement.body);
	}
	...
	{
		{
			const fuSwitchValue = this.#visitExpr(value, true);
			let symbol;
			let klass;
			if (fuSwitchValue instanceof FuLiteralNull) {
			}
			else if ((symbol = fuSwitchValue) instanceof FuSymbolReference && (klass = symbol.symbol) instanceof FuClass)
				this.#checkIsHierarchy(switchPtr, statement.value, klass, value, "case", "always match", "never match");
			else if (fuSwitchValue instanceof FuVar) {
				const def = this.#visitExpr(value, true);
				this.#checkIsVar(statement.value, def, def, "case", "always match", "never match");
			}
			else
				this.#reportError(value, "Expected 'case Class'");
		}
	}
	...
	#visitSwitch(statement)
	{
		...
		if (statement.value != this.#poison && statement.value.type != this.#poison) {
			{
				const fuSwitchValue = statement.value.type;
				let i;
				let klass;
				if ((i = fuSwitchValue) instanceof FuIntegerType && i.id != FuId.LONG_TYPE) {
				}
				else if (fuSwitchValue instanceof FuEnum) {
				}
				else if ((klass = fuSwitchValue) instanceof FuClassType && !(klass instanceof FuStorageType)) {
				}
				else
...
export class GenC extends GenCCpp
{
	...
	writeLocalName(symbol, parent)
	{
		...
		if ((forEach = symbol.parent) instanceof FuForeach) {
			...
			{
				const fuSwitchValue = klass.class.id;
				let elementClass;
				if (fuSwitchValue == FuId.STRING_CLASS || (fuSwitchValue == FuId.LIST_CLASS && (!(klass.getElementType() instanceof FuStorageType) || ((elementClass = klass.getElementType()) instanceof FuClassType && elementClass.class.id == FuId.LIST_CLASS)))) {
				...
				else if (fuSwitchValue == FuId.ARRAY_STORAGE_CLASS) {
	...
	#writeNumberToGPointerCast(type, expr)
	{
		...
		case FuId.DOUBLE_TYPE:
			...
			break;
		default:
			console.assert(type instanceof FuIntegerType || type instanceof FuEnum);
	...
	#writeGPointerCast(type, expr)
	{
		if (type instanceof FuNumericType || type instanceof FuEnum)
			this.#writeNumberToGPointerCast(type, expr);
	...
	#startDictionaryLookup(function_, obj, key)
	{
		this.write(function_);
		this.writeChar(40);
		obj.accept(this, FuPriority.ARGUMENT);
		this.write(", ");
		let type = obj.type.asClassType().getKeyType();
		if (type instanceof FuNumericType || type instanceof FuEnum)
			this.#writeNumberToGPointerCast(type, key);
		else if (key.type instanceof FuStorageType)
			this.#writeAddressOf(key);
		else
	...
	writePrintfNotInterpolated(args, newLine)
	{
		this.write("\"%");
		{
			const fuSwitchValue = args[0].type;
			if (fuSwitchValue instanceof FuIntegerType) {
				const intType = args[0].type;
				this.#writePrintfIntPrefix(intType.id);
				this.writeChar(100);
			}
			else if (fuSwitchValue instanceof FuFloatingType)
				this.writeChar(103);
			else
				this.writeChar(115);
		}
	...
	#writeMathClampMaxMin(type, method, args)
	{
		switch (type.id) {
		case FuId.INT_TYPE:
	...
	writeCallExpr(type, obj, method, args, parent)
	{
		...
		case FuId.INT_TRY_PARSE:
			...
			this.#writeTryParse("FuInt", obj, args);
			break;
		case FuId.N_INT_TRY_PARSE:
			this.include("limits.h");
			this.#nIntFunctions.add(FuId.INT_TRY_PARSE);
			this.#writeTryParse("FuNInt", obj, args);
			break;
		case FuId.LONG_TRY_PARSE:
		...
		case FuId.DICTIONARY_CONTAINS_KEY:
			this.#startDictionaryLookup("g_hash_table_contains", obj, args[0]);
			this.writeChar(41);
			break;
		case FuId.HASH_SET_REMOVE:
		case FuId.DICTIONARY_REMOVE:
			this.#startDictionaryLookup("g_hash_table_remove", obj, args[0]);
			this.writeChar(41);
			break;
		...
		case FuId.SORTED_DICTIONARY_CONTAINS_KEY:
			this.#startDictionaryLookup("g_tree_lookup_extended", obj, args[0]);
			this.write(", NULL, NULL)");
			break;
		case FuId.SORTED_SET_REMOVE:
		case FuId.SORTED_DICTIONARY_REMOVE:
			this.#startDictionaryLookup("g_tree_remove", obj, args[0]);
			this.writeChar(41);
			break;
		...
		case FuId.MATH_CLAMP:
			if (this.#writeMathClampMaxMin(type, method, args)) {
				this.includeMath();
				this.write(type.id == FuId.DOUBLE_TYPE ? "fmin(fmax(" : "fminf(fmaxf(");
	...
	#writeDictionaryIndexing(function_, expr, parent)
	{
		...
		if (valueType instanceof FuIntegerType && valueType.id != FuId.LONG_TYPE) {
			...
			this.#startDictionaryLookup(function_, expr.left, expr.right);
			this.write("))");
		}
	...
	#writeIntLibrary(klassName, type, methods)
	{
		...
		if (methods.has(FuId.INT_TRY_PARSE)) {
			...
			if (klassName == "Int") {
				...
				this.writeLine("#if PTRDIFF_MAX == LONG_MAX");
				this.writeLine("\tstrtol");
				this.writeLine("#else");
				this.writeLine("\tstrtoll");
				this.writeLine("#endif");
				this.writeLine("\t(str, &end, base);");
...
export class GenCpp extends GenCCpp
{
	...
	#writeCollectionType(klass)
	{
		...
		case FuId.PRIORITY_QUEUE_CLASS:
			...
			this.include("queue");
			this.write("std::priority_queue<FuPriorityQueueEntry<");
			this.writeType(elementType, false);
			this.write(", ");
			this.writeType(klass.typeArg1, false);
			this.write(">>");
			return;
		case FuId.HASH_SET_CLASS:
	...
	#writePop(obj, parent, p, front, frontSuffix = "")
	{
		if (parent == FuPriority.STATEMENT) {
	...
	writeCallExpr(type, obj, method, args, parent)
	{
		...
		case FuId.PRIORITY_QUEUE_DEQUEUE:
			this.#writePop(obj, parent, 113, "top", "().get");
			break;
		case FuId.PRIORITY_QUEUE_ENQUEUE:
			this.writeMethodCall(obj, "emplace", args[0], args[1]);
			break;
	...
	{
		if (expr.type.id == FuId.STRING_STORAGE_TYPE) {
			this.write("std::string fuSwitchValue{");
			expr.accept(this, FuPriority.ARGUMENT);
			this.writeChar(125);
		}
		else
	...
	writeProgram(program, outputFile, namespace)
	{
		...
		if (this.#hasPriorityQueue) {
			this.writeLine("template <class TElement, class TPriority>");
			this.writeLine("class FuPriorityQueueEntry");
			this.openBlock();
			this.writeLine("TPriority priority;");
			this.writeLine("TElement element;");
			this.indent--;
			this.writeLine("public:");
			this.indent++;
			this.writeLine("FuPriorityQueueEntry(const TElement &element, const TPriority &priority) : priority(priority), element(element) {}");
			this.writeLine("const TElement &get() const { return element; }");
...
export class GenJava extends GenTyped
{
	...
	#writeJavaType(type, promote, needClass)
	{
		if (type instanceof FuNumericType) {
			...
			case FuId.PRIORITY_QUEUE_CLASS:
				this.#hasPriorityQueue = true;
				this.include("java.util.PriorityQueue");
				this.write("PriorityQueue<FuPriorityQueueEntry<");
				this.#writeJavaType(klass.typeArg0, false, true);
				this.write(", ");
				this.#writeJavaType(klass.typeArg1, false, true);
				this.write(">>");
				break;
			case FuId.HASH_SET_CLASS:
	...
	writeSwitchCaseValue(statement, value)
	{
		...
		if ((symbol = value) instanceof FuSymbolReference) {
			...
			let klass;
			if ((klass = symbol.symbol) instanceof FuClass) {
				this.write(klass.name);
				this.write(" _");
				return;
			}
	...
	#writePriorityQueue()
	{
		this.#createJavaFile("FuPriorityQueueEntry");
		this.writeLine("class FuPriorityQueueEntry<TElement, TPriority extends Comparable<TPriority>>");
		this.writeLine("\timplements Comparable<FuPriorityQueueEntry<TElement, TPriority>>");
		this.openBlock();
		this.writeLine("private final TElement element;");
		this.writeLine("private final TPriority priority;");
		this.writeNewLine();
		this.writeLine("public FuPriorityQueueEntry(TElement element, TPriority priority)");
		this.openBlock();
		this.writeLine("this.element = element;");
		this.writeLine("this.priority = priority;");
		this.closeBlock();
		this.writeNewLine();
		this.writeLine("public TElement get()");
		this.openBlock();
		this.writeLine("return element;");
		this.closeBlock();
		this.writeNewLine();
		this.writeLine("public int compareTo(FuPriorityQueueEntry<TElement, TPriority> o)");
		this.openBlock();
		this.writeLine("return priority.compareTo(o.priority);");
		this.closeBlock();
		this.closeBlock();
		this.closeFile();
	}
...
export class GenJsNoModule extends GenBase
{
	...
	visitForeach(statement)
	{
		...
		case FuId.SORTED_SET_CLASS:
			...
			this.write(" of ");
			{
				const fuSwitchValue = klass.getElementType();
				if (fuSwitchValue instanceof FuNumericType) {
		...
		case FuId.ORDERED_DICTIONARY_CLASS:
			...
			if (klass.class.id == FuId.ORDERED_DICTIONARY_CLASS)
				...
				{
					const fuSwitchValue = statement.getVar().type;
					if (fuSwitchValue instanceof FuStringType) {
						if (klass.class.id == FuId.SORTED_DICTIONARY_CLASS)
							this.write(".sort((a, b) => a[0].localeCompare(b[0]))");
					}
					else if (fuSwitchValue instanceof FuNumericType || fuSwitchValue instanceof FuEnum) {
	...
	writeSwitchVar(expr)
	{
		this.write("const fuSwitchValue = ");
		expr.accept(this, FuPriority.ARGUMENT);
	}
	...
	{
		let caseVar;
		if (kase != null && (caseVar = kase.values[0]) instanceof FuVar) {
			this.writeChar(32);
			this.openBlock();
...
export class GenTs extends GenJs
{
	...
	#writeType(type, readOnly = false)
	{
		if (type instanceof FuNumericType)
			...
			{
				const fuSwitchValue = klass.class.id;
				if (fuSwitchValue == FuId.STRING_CLASS)
					this.write("string");
				else if ((fuSwitchValue == FuId.ARRAY_PTR_CLASS && !(klass.getElementType() instanceof FuNumericType)) || (fuSwitchValue == FuId.ARRAY_STORAGE_CLASS && !(klass.getElementType() instanceof FuNumericType)) || fuSwitchValue == FuId.LIST_CLASS || fuSwitchValue == FuId.QUEUE_CLASS || fuSwitchValue == FuId.STACK_CLASS) {
					if (readOnly)
						this.write("readonly ");
...
export class GenPySwift extends GenBase
{
	...
	writeListAppend(obj, args)
	{
		...
		if (args.length == 0)
			...
			this.writeElementCoerced(elementType, args[0]);
		this.writeChar(41);
	}
...
export class GenSwift extends GenPySwift
{
	...
	{
		let loop;
		if ((loop = symbol.parent) instanceof FuForeach) {
			let klass = loop.collection.type;
			if (klass.class.id == FuId.STRING_CLASS) {
				this.write("Int(");
				this.#writeCamelCaseNotKeyword(symbol.name);
			...
			let elementType = symbol == loop.first ? klass.getElementType() : klass.getValueType();
			if (elementType.id == FuId.INT_TYPE) {
				this.write("Int(");
				this.#writeCamelCaseNotKeyword(symbol.name);
				this.writeChar(41);
				return;
	...
	{
		if (part.widthExpr != null && part.argument.type instanceof FuClassType) {
			this.include("Foundation");
			this.#stringPad = true;
	...
	{
		if (expr.type.id != FuId.INT_TYPE)
			return false;
		let unary;
		if ((unary = expr) instanceof FuUnaryExpr && (unary.op == FuToken.INCREMENT || unary.op == FuToken.DECREMENT))
			expr = unary.inner;
	...
	writeCoercedInternal(type, expr, parent)
	{
		if ((type instanceof FuNumericType && !(expr instanceof FuLiteral) && this.getTypeId(type, false) != this.getTypeId(expr.type, expr instanceof FuBinaryExpr && !expr.isIndexing())) || GenSwift.#isIntIndexing(expr)) {
			this.#writePromotedType(type);
			this.writeChar(40);
			let call;
		...
		if (expr.type.id != FuId.BOOL_TYPE) {
			...
			if (binary.op == FuToken.PLUS || binary.op == FuToken.MINUS || binary.op == FuToken.ASTERISK || binary.op == FuToken.SLASH || binary.op == FuToken.MOD || binary.op == FuToken.AND || binary.op == FuToken.OR || binary.op == FuToken.XOR || (binary.op == FuToken.SHIFT_LEFT && expr == binary.left) || (binary.op == FuToken.SHIFT_RIGHT && expr == binary.left)) {
				if (!(expr instanceof FuLiteral)) {
					...
					if (type != expr.type || GenSwift.#isIntIndexing(expr)) {
						this.writeCoerced(type, expr, parent);
						return;
					}
				}
	...
	#writeSwiftCaseValue(statement, value)
	{
		let symbol;
		let klass;
		let when1;
		if ((symbol = value) instanceof FuSymbolReference && (klass = symbol.symbol) instanceof FuClass) {
			this.write("is ");
			this.write(klass.name);
		}
		else if (value instanceof FuVar) {
			...
			this.#writeCamelCaseNotKeyword(def.name);
			this.write(" as ");
			this.#writePromotedType(def.type);
		}
		else if ((when1 = value) instanceof FuBinaryExpr && when1.op == FuToken.WHEN) {
			this.#writeSwiftCaseValue(statement, when1.left);
			this.write(" where ");
			this.writeExpr(when1.right, FuPriority.ARGUMENT);
		}
		else
			this.writeCoerced(statement.value.type, value, FuPriority.ARGUMENT);
	}
	...
	visitSwitch(statement)
	{
		...
		for (const kase of statement.cases) {
			...
			for (let i = 0; i < kase.values.length; i++) {
				this.writeComma(i);
				this.#writeSwiftCaseValue(statement, kase.values[i]);
			}
			this.writeCharLine(58);
	...
	writeMethod(method)
	{
		...
		if (method.id == FuId.CLASS_TO_STRING)
			...
			if (method.type.id != FuId.VOID_TYPE) {
				this.write(" -> ");
				this.#writePromotedType(method.type);
			}
		}
		this.writeNewLine();
		this.openBlock();
...
export class GenPy extends GenPySwift
{
	...
	#writeTypeAnnotation(type, nullable = false)
	{
		if (type instanceof FuIntegerType)
			...
			case FuId.QUEUE_CLASS:
				...
				break;
			case FuId.PRIORITY_QUEUE_CLASS:
				this.write("list[tuple[");
				this.#writeTypeAnnotation(klass.typeArg1);
				this.write(", ");
				this.#writeTypeAnnotation(klass.typeArg0);
				this.write("]]");
				break;
			case FuId.HASH_SET_CLASS:
			case FuId.SORTED_SET_CLASS:
	...
	writeCallExpr(type, obj, method, args, parent)
	{
		...
		case FuId.PRIORITY_QUEUE_DEQUEUE:
			this.include("heapq");
			this.write("heapq.heappop(");
			obj.accept(this, FuPriority.ARGUMENT);
			this.write(")[1]");
			break;
		case FuId.PRIORITY_QUEUE_ENQUEUE:
			this.include("heapq");
			this.write("heapq.heappush(");
			obj.accept(this, FuPriority.ARGUMENT);
			this.write(", (");
			args[1].accept(this, FuPriority.ARGUMENT);
			this.write(", ");
	...
	#writePyCaseValue(value)
	{
		...
		if ((symbol = value) instanceof FuSymbolReference && (klass = symbol.symbol) instanceof FuClass) {
			...
			this.#writePyCaseValue(when1.left);
			this.write(" if ");
			when1.right.accept(this, FuPriority.ARGUMENT);
		}
		else
	...
	visitSwitch(statement)
	{
		...
		for (const kase of statement.cases) {
			...
			for (const caseValue of kase.values) {
				this.write(op);
				this.#writePyCaseValue(caseValue);
				op = " | ";
			}
