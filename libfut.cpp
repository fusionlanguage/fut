...
bool FuExpr::isLocalReference() const
{
	return false;
}
...
bool FuSymbolReference::isLocalReference() const
{
...
FuSystem::FuSystem()
{
	...
	this->typeParam1->id = FuId::typeParam1;
	this->typeParam1->name = "T";
	...
	priorityQueueClass->addMethod(this->voidType, FuId::priorityQueueEnqueue, "Enqueue", true, FuVar::new_(this->typeParam0, "element"), FuVar::new_(this->typeParam1, "priority"));
	...
	utf8EncodingClass->add(FuMethodGroup::new_(FuMethod::new_(nullptr, FuVisibility::public_, FuCallType::normal, futemp4, FuId::uTF8GetBytesAlloc, "GetBytes", false, FuVar::new_(this->stringPtrType, "str")), FuMethod::new_(nullptr, FuVisibility::public_, FuCallType::normal, this->voidType, FuId::uTF8GetBytesCopy, "GetBytes", false, FuVar::new_(this->stringPtrType, "str"), FuVar::new_(futemp5, "bytes"), FuVar::new_(this->nIntType, "byteIndex"))));
	...
	addVirtualAssertFalseMethod(jsonElementClass.get(), futemp10, FuId::jsonElementGetObject, "GetObject");
	std::shared_ptr<FuClassType> futemp11 = std::make_shared<FuClassType>();
	futemp11->class_ = listClass;
	futemp11->typeArg0 = jsonElementPtr;
	addVirtualAssertFalseMethod(jsonElementClass.get(), futemp11, FuId::jsonElementGetArray, "GetArray");
	...
	std::shared_ptr<FuDynamicPtrType> futemp12 = std::make_shared<FuDynamicPtrType>();
	futemp12->nullable = true;
	futemp12->class_ = jsonElementClass.get();
	std::shared_ptr<FuMethod> jsonParserTryParseMethod = FuMethod::new_(nullptr, FuVisibility::internal, FuCallType::normal, futemp12, FuId::none, "TryParse", true);
	{
		...
		std::shared_ptr<FuStorageType> futemp13 = std::make_shared<FuStorageType>();
		futemp13->class_ = jsonParserClass.get();
		std::shared_ptr<FuVar> parserVar = FuVar::new_(futemp13, "parser");
		...
		std::shared_ptr<FuReturn> futemp14 = std::make_shared<FuReturn>();
		futemp14->value = call;
		methodBlock->statements.push_back(futemp14);
	...
	jsonObjectValueField->type = futemp13;
	...
	std::shared_ptr<FuClassType> futemp14 = std::make_shared<FuClassType>();
	futemp14->class_ = dictionaryClass;
	futemp14->typeArg0 = this->stringStorageType;
	futemp14->typeArg1 = jsonElementPtr;
	addJsonGetMethod(jsonObjectClass.get(), futemp14, "GetObject", jsonObjectValueField.get());
	...
	std::shared_ptr<FuStorageType> futemp15 = std::make_shared<FuStorageType>();
	...
	jsonArrayValueField->type = futemp15;
	...
	futemp16->class_ = listClass;
	futemp16->typeArg0 = jsonElementPtr;
	addJsonGetMethod(jsonArrayClass.get(), futemp16, "GetArray", jsonArrayValueField.get());
	...
	std::shared_ptr<FuMethod> jsonParserSkipWhitespaceMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, this->boolType, FuId::none, "SkipWhitespace", true);
	{
		...
		std::shared_ptr<FuWhile> futemp17 = std::make_shared<FuWhile>();
		futemp17->cond = newOffsetLessInputLength(offsetField.get(), inputLengthField.get());
		futemp17->body = whileBlock;
		methodBlock->statements.push_back(futemp17);
		...
		addCaseCharValue(kase, ' ');
		std::shared_ptr<FuBreak> futemp18 = std::make_shared<FuBreak>();
		futemp18->loopOrSwitch = switch_.get();
		kase->body.push_back(futemp18);
		...
		whileBlock->statements.push_back(futemp19);
		std::shared_ptr<FuLiteralFalse> futemp20 = std::make_shared<FuLiteralFalse>();
		futemp20->type = this->boolType;
		std::shared_ptr<FuReturn> futemp21 = std::make_shared<FuReturn>();
		futemp21->value = futemp20;
	...
	futemp17->class_ = jsonObjectClass.get();
	std::shared_ptr<FuMethod> jsonParserParseObjectMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, futemp17, FuId::none, "ParseObject", true);
	std::shared_ptr<FuDynamicPtrType> futemp18 = std::make_shared<FuDynamicPtrType>();
	futemp18->nullable = true;
	futemp18->class_ = jsonStringClass.get();
	std::shared_ptr<FuMethod> jsonParserParseStringMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, futemp18, FuId::none, "ParseString", true);
	std::shared_ptr<FuDynamicPtrType> futemp19 = std::make_shared<FuDynamicPtrType>();
	futemp19->nullable = true;
	futemp19->class_ = jsonElementClass.get();
	std::shared_ptr<FuMethod> jsonParserParseWhitespaceAndElementMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, futemp19, FuId::none, "ParseWhitespaceAndElement", true);
	{
		std::shared_ptr<FuBlock> methodBlock = std::make_shared<FuBlock>();
		std::shared_ptr<FuPostfixExpr> futemp20 = std::make_shared<FuPostfixExpr>();
		futemp20->inner = newSymbolReference(offsetField.get());
		futemp20->op = FuToken::increment;
		methodBlock->statements.push_back(futemp20);
		std::shared_ptr<FuPrefixExpr> futemp21 = std::make_shared<FuPrefixExpr>();
		futemp21->type = this->boolType;
		futemp21->op = FuToken::exclamationMark;
		futemp21->inner = newCall(jsonParserSkipWhitespaceMethod.get());
		std::shared_ptr<FuIf> futemp22 = std::make_shared<FuIf>();
		futemp22->cond = futemp21;
		futemp22->onTrue = newReturnNull();
		futemp22->onFalse = nullptr;
		methodBlock->statements.push_back(futemp22);
		...
		std::shared_ptr<FuLiteralChar> futemp23 = std::make_shared<FuLiteralChar>();
		futemp23->type = this->intType;
		futemp23->value = '}';
		std::shared_ptr<FuBinaryExpr> futemp24 = std::make_shared<FuBinaryExpr>();
		futemp24->type = this->boolType;
		futemp24->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp24->op = FuToken::equal;
		futemp24->right = futemp23;
		std::shared_ptr<FuIf> futemp25 = std::make_shared<FuIf>();
		futemp25->cond = futemp24;
		futemp25->onTrue = ifBlock;
		futemp25->onFalse = nullptr;
		methodBlock->statements.push_back(futemp25);
		std::shared_ptr<FuPostfixExpr> futemp26 = std::make_shared<FuPostfixExpr>();
		futemp26->inner = newSymbolReference(offsetField.get());
		futemp26->op = FuToken::increment;
		ifBlock->statements.push_back(futemp26);
		std::shared_ptr<FuReturn> futemp27 = std::make_shared<FuReturn>();
		futemp27->value = newSymbolReference(resultVar.get());
		ifBlock->statements.push_back(futemp27);
		std::shared_ptr<FuBlock> whileBlock = std::make_shared<FuBlock>();
		whileBlock->setCompletesNormally(true);
		std::shared_ptr<FuLiteralChar> futemp28 = std::make_shared<FuLiteralChar>();
		futemp28->type = this->intType;
		futemp28->value = '"';
		std::shared_ptr<FuBinaryExpr> futemp29 = std::make_shared<FuBinaryExpr>();
		futemp29->type = this->boolType;
		futemp29->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp29->op = FuToken::equal;
		futemp29->right = futemp28;
		std::shared_ptr<FuWhile> futemp30 = std::make_shared<FuWhile>();
		futemp30->cond = futemp29;
		futemp30->body = whileBlock;
		methodBlock->statements.push_back(futemp30);
		std::shared_ptr<FuDynamicPtrType> futemp31 = std::make_shared<FuDynamicPtrType>();
		futemp31->nullable = true;
		futemp31->class_ = jsonStringClass.get();
		std::shared_ptr<FuVar> keyVar = FuVar::new_(futemp31, "key", newCall(jsonParserParseStringMethod.get()));
		whileBlock->statements.push_back(keyVar);
		whileBlock->add(keyVar);
		std::shared_ptr<FuLiteralNull> futemp32 = std::make_shared<FuLiteralNull>();
		futemp32->type = this->nullType;
		std::shared_ptr<FuBinaryExpr> futemp33 = std::make_shared<FuBinaryExpr>();
		futemp33->type = this->boolType;
		futemp33->left = newSymbolReference(keyVar.get());
		futemp33->op = FuToken::equal;
		futemp33->right = futemp32;
		std::shared_ptr<FuPrefixExpr> futemp34 = std::make_shared<FuPrefixExpr>();
		futemp34->type = this->boolType;
		futemp34->op = FuToken::exclamationMark;
		futemp34->inner = newCall(jsonParserSkipWhitespaceMethod.get());
		...
		futemp36->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp36->op = FuToken::notEqual;
		futemp36->right = futemp35;
		std::shared_ptr<FuBinaryExpr> futemp37 = std::make_shared<FuBinaryExpr>();
		futemp37->type = this->boolType;
		futemp37->left = futemp34;
		futemp37->op = FuToken::condOr;
		futemp37->right = futemp36;
		std::shared_ptr<FuBinaryExpr> futemp38 = std::make_shared<FuBinaryExpr>();
		futemp38->type = this->boolType;
		futemp38->left = futemp33;
		futemp38->op = FuToken::condOr;
		futemp38->right = futemp37;
		std::shared_ptr<FuIf> futemp39 = std::make_shared<FuIf>();
		futemp39->cond = futemp38;
		futemp39->onTrue = newReturnNull();
		futemp39->onFalse = nullptr;
		whileBlock->statements.push_back(futemp39);
		std::shared_ptr<FuPostfixExpr> futemp40 = std::make_shared<FuPostfixExpr>();
		futemp40->inner = newSymbolReference(offsetField.get());
		futemp40->op = FuToken::increment;
		whileBlock->statements.push_back(futemp40);
		std::shared_ptr<FuDynamicPtrType> futemp41 = std::make_shared<FuDynamicPtrType>();
		futemp41->nullable = true;
		futemp41->class_ = jsonElementClass.get();
		std::shared_ptr<FuVar> valueVar = FuVar::new_(futemp41, "value", newCall(jsonParserParseWhitespaceAndElementMethod.get()));
		whileBlock->statements.push_back(valueVar);
		whileBlock->add(valueVar);
		std::shared_ptr<FuLiteralNull> futemp42 = std::make_shared<FuLiteralNull>();
		futemp42->type = this->nullType;
		std::shared_ptr<FuBinaryExpr> futemp43 = std::make_shared<FuBinaryExpr>();
		futemp43->type = this->boolType;
		futemp43->left = newSymbolReference(valueVar.get());
		futemp43->op = FuToken::equal;
		futemp43->right = futemp42;
		std::shared_ptr<FuPrefixExpr> futemp44 = std::make_shared<FuPrefixExpr>();
		futemp44->type = this->boolType;
		futemp44->op = FuToken::exclamationMark;
		futemp44->inner = newCall(jsonParserSkipWhitespaceMethod.get());
		std::shared_ptr<FuBinaryExpr> futemp45 = std::make_shared<FuBinaryExpr>();
		futemp45->type = this->boolType;
		futemp45->left = futemp43;
		futemp45->op = FuToken::condOr;
		futemp45->right = futemp44;
		std::shared_ptr<FuIf> futemp46 = std::make_shared<FuIf>();
		futemp46->cond = futemp45;
		futemp46->onTrue = newReturnNull();
		futemp46->onFalse = nullptr;
		whileBlock->statements.push_back(futemp46);
		std::shared_ptr<FuBinaryExpr> futemp47 = std::make_shared<FuBinaryExpr>();
		futemp47->type = jsonElementPtr;
		futemp47->left = newMemberReference(newSymbolReference(resultVar.get()), jsonObjectValueField.get());
		futemp47->op = FuToken::leftBracket;
		futemp47->right = newMemberReference(newSymbolReference(keyVar.get()), jsonStringValueField.get());
		std::shared_ptr<FuBinaryExpr> futemp48 = std::make_shared<FuBinaryExpr>();
		futemp48->type = jsonElementPtr;
		futemp48->left = futemp47;
		futemp48->op = FuToken::assign;
		futemp48->right = newSymbolReference(valueVar.get());
		whileBlock->statements.push_back(futemp48);
		std::shared_ptr<FuPostfixExpr> futemp49 = std::make_shared<FuPostfixExpr>();
		futemp49->type = this->nIntType;
		futemp49->inner = newSymbolReference(offsetField.get());
		futemp49->op = FuToken::increment;
		std::shared_ptr<FuBinaryExpr> futemp50 = std::make_shared<FuBinaryExpr>();
		futemp50->type = this->intType;
		futemp50->left = newSymbolReference(inputField.get());
		futemp50->op = FuToken::leftBracket;
		futemp50->right = futemp49;
		std::shared_ptr<FuSwitch> switch_ = std::make_shared<FuSwitch>();
		switch_->value = futemp50;
		switch_->cases.emplace_back();
		addCaseCharValue(&switch_->cases[0], ',');
		std::shared_ptr<FuBreak> futemp51 = std::make_shared<FuBreak>();
		futemp51->loopOrSwitch = switch_.get();
		switch_->cases[0].body.push_back(futemp51);
		switch_->cases.emplace_back();
		addCaseCharValue(&switch_->cases[1], '}');
		std::shared_ptr<FuReturn> futemp52 = std::make_shared<FuReturn>();
		futemp52->value = newSymbolReference(resultVar.get());
		switch_->cases[1].body.push_back(futemp52);
		switch_->defaultBody.push_back(newReturnNull());
		whileBlock->statements.push_back(switch_);
		std::shared_ptr<FuPrefixExpr> futemp53 = std::make_shared<FuPrefixExpr>();
		futemp53->type = this->boolType;
		futemp53->op = FuToken::exclamationMark;
		futemp53->inner = newCall(jsonParserSkipWhitespaceMethod.get());
		std::shared_ptr<FuIf> futemp54 = std::make_shared<FuIf>();
		futemp54->cond = futemp53;
		futemp54->onTrue = newReturnNull();
		futemp54->onFalse = nullptr;
		whileBlock->statements.push_back(futemp54);
	...
	futemp20->class_ = jsonArrayClass.get();
	std::shared_ptr<FuMethod> jsonParserParseArrayMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, futemp20, FuId::none, "ParseArray", true);
	std::shared_ptr<FuDynamicPtrType> futemp21 = std::make_shared<FuDynamicPtrType>();
	futemp21->nullable = true;
	futemp21->class_ = jsonElementClass.get();
	std::shared_ptr<FuMethod> jsonParserParseElementMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, futemp21, FuId::none, "ParseElement", true);
	{
		std::shared_ptr<FuBlock> methodBlock = std::make_shared<FuBlock>();
		std::shared_ptr<FuPostfixExpr> futemp22 = std::make_shared<FuPostfixExpr>();
		futemp22->inner = newSymbolReference(offsetField.get());
		futemp22->op = FuToken::increment;
		methodBlock->statements.push_back(futemp22);
		std::shared_ptr<FuPrefixExpr> futemp23 = std::make_shared<FuPrefixExpr>();
		futemp23->type = this->boolType;
		futemp23->op = FuToken::exclamationMark;
		futemp23->inner = newCall(jsonParserSkipWhitespaceMethod.get());
		std::shared_ptr<FuIf> futemp24 = std::make_shared<FuIf>();
		futemp24->cond = futemp23;
		futemp24->onTrue = newReturnNull();
		...
		methodBlock->add(resultVar);
		std::shared_ptr<FuBlock> ifBlock = std::make_shared<FuBlock>();
		std::shared_ptr<FuLiteralChar> futemp25 = std::make_shared<FuLiteralChar>();
		futemp25->type = this->intType;
		futemp25->value = ']';
		std::shared_ptr<FuBinaryExpr> futemp26 = std::make_shared<FuBinaryExpr>();
		futemp26->type = this->boolType;
		futemp26->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp26->op = FuToken::equal;
		futemp26->right = futemp25;
		std::shared_ptr<FuIf> futemp27 = std::make_shared<FuIf>();
		futemp27->cond = futemp26;
		futemp27->onTrue = ifBlock;
		futemp27->onFalse = nullptr;
		methodBlock->statements.push_back(futemp27);
		...
		std::shared_ptr<FuReturn> futemp29 = std::make_shared<FuReturn>();
		futemp29->value = newSymbolReference(resultVar.get());
		ifBlock->statements.push_back(futemp29);
		std::shared_ptr<FuBlock> doWhileBlock = std::make_shared<FuBlock>();
		doWhileBlock->setCompletesNormally(true);
		std::shared_ptr<FuDoWhile> futemp30 = std::make_shared<FuDoWhile>();
		futemp30->body = doWhileBlock;
		futemp30->cond = newCall(jsonParserSkipWhitespaceMethod.get());
		methodBlock->statements.push_back(futemp30);
		std::shared_ptr<FuDynamicPtrType> futemp31 = std::make_shared<FuDynamicPtrType>();
		futemp31->nullable = true;
		futemp31->class_ = jsonElementClass.get();
		std::shared_ptr<FuVar> elementVar = FuVar::new_(futemp31, "element", newCall(jsonParserParseElementMethod.get()));
		...
		std::shared_ptr<FuBinaryExpr> futemp33 = std::make_shared<FuBinaryExpr>();
		futemp33->type = this->boolType;
		futemp33->left = newSymbolReference(elementVar.get());
		futemp33->op = FuToken::equal;
		futemp33->right = futemp32;
		std::shared_ptr<FuPrefixExpr> futemp34 = std::make_shared<FuPrefixExpr>();
		futemp34->type = this->boolType;
		...
		futemp35->left = futemp33;
		futemp35->op = FuToken::condOr;
		futemp35->right = futemp34;
		std::shared_ptr<FuIf> futemp36 = std::make_shared<FuIf>();
		futemp36->cond = futemp35;
		futemp36->onTrue = newReturnNull();
		futemp36->onFalse = nullptr;
		doWhileBlock->statements.push_back(futemp36);
		...
		std::shared_ptr<FuPostfixExpr> futemp37 = std::make_shared<FuPostfixExpr>();
		futemp37->type = this->nIntType;
		futemp37->inner = newSymbolReference(offsetField.get());
		futemp37->op = FuToken::increment;
		std::shared_ptr<FuBinaryExpr> futemp38 = std::make_shared<FuBinaryExpr>();
		futemp38->type = this->intType;
		futemp38->left = newSymbolReference(inputField.get());
		futemp38->op = FuToken::leftBracket;
		futemp38->right = futemp37;
		std::shared_ptr<FuSwitch> switch_ = std::make_shared<FuSwitch>();
		switch_->value = futemp38;
		switch_->cases.emplace_back();
		addCaseCharValue(&switch_->cases[0], ',');
		std::shared_ptr<FuBreak> futemp39 = std::make_shared<FuBreak>();
		futemp39->loopOrSwitch = switch_.get();
		switch_->cases[0].body.push_back(futemp39);
		switch_->cases.emplace_back();
		addCaseCharValue(&switch_->cases[1], ']');
		std::shared_ptr<FuReturn> futemp40 = std::make_shared<FuReturn>();
		futemp40->value = newSymbolReference(resultVar.get());
		switch_->cases[1].body.push_back(futemp40);
		switch_->defaultBody.push_back(newReturnNull());
	...
	jsonParserClass->add(jsonParserParseArrayMethod);
	{
		...
		futemp22->op = FuToken::increment;
		methodBlock->statements.push_back(futemp22);
		std::shared_ptr<FuStorageType> futemp23 = std::make_shared<FuStorageType>();
		futemp23->class_ = stringWriterClass.get();
		std::shared_ptr<FuVar> resultVar = FuVar::new_(futemp23, "result");
		methodBlock->statements.push_back(resultVar);
		methodBlock->add(resultVar);
		...
		std::shared_ptr<FuBinaryExpr> futemp24 = std::make_shared<FuBinaryExpr>();
		futemp24->left = newSymbolReference(jsonStringValueField.get());
		futemp24->op = FuToken::assign;
		futemp24->right = newMethodCall(resultVar.get(), stringWriterToStringMethod.get());
		init->items.push_back(futemp24);
		std::shared_ptr<FuDynamicPtrType> futemp25 = std::make_shared<FuDynamicPtrType>();
		futemp25->class_ = jsonStringClass.get();
		std::shared_ptr<FuPrefixExpr> futemp26 = std::make_shared<FuPrefixExpr>();
		futemp26->type = futemp25;
		futemp26->op = FuToken::new_;
		futemp26->inner = init;
		std::shared_ptr<FuReturn> futemp27 = std::make_shared<FuReturn>();
		futemp27->value = futemp26;
		switch_->cases[1].body.push_back(futemp27);
		switch_->cases.emplace_back();
		...
		futemp28->left = newSymbolReference(offsetField.get());
		futemp28->op = FuToken::greaterOrEqual;
		futemp28->right = newSymbolReference(inputLengthField.get());
		std::shared_ptr<FuIf> futemp29 = std::make_shared<FuIf>();
		futemp29->cond = futemp28;
		futemp29->onTrue = newReturnNull();
		futemp29->onFalse = nullptr;
		switch_->cases[2].body.push_back(futemp29);
		...
		addCaseCharValue(&switch2->cases[0], '\\');
		addCaseCharValue(&switch2->cases[0], '/');
		std::shared_ptr<FuPostfixExpr> futemp30 = std::make_shared<FuPostfixExpr>();
		futemp30->type = this->nIntType;
		futemp30->inner = newSymbolReference(offsetField.get());
		futemp30->op = FuToken::increment;
		std::shared_ptr<FuBinaryExpr> futemp31 = std::make_shared<FuBinaryExpr>();
		futemp31->type = this->nIntType;
		futemp31->left = newSymbolReference(startOffsetVar.get());
		futemp31->op = FuToken::assign;
		futemp31->right = futemp30;
		switch2->cases[0].body.push_back(futemp31);
		std::shared_ptr<FuContinue> futemp32 = std::make_shared<FuContinue>();
		futemp32->loop = while_.get();
		switch2->cases[0].body.push_back(futemp32);
		...
		futemp33->value = '\n';
		addJsonEscape(switch2.get(), 'n', resultVar.get(), textWriterWriteCharMethod.get(), futemp33);
		std::shared_ptr<FuLiteralChar> futemp34 = std::make_shared<FuLiteralChar>();
		futemp34->type = this->intType;
		futemp34->value = '\r';
		addJsonEscape(switch2.get(), 'r', resultVar.get(), textWriterWriteCharMethod.get(), futemp34);
		std::shared_ptr<FuLiteralChar> futemp35 = std::make_shared<FuLiteralChar>();
		futemp35->type = this->intType;
		futemp35->value = '\t';
		addJsonEscape(switch2.get(), 't', resultVar.get(), textWriterWriteCharMethod.get(), futemp35);
		switch2->cases.emplace_back();
		addCaseCharValue(&switch2->cases[6], 'u');
		std::shared_ptr<FuBinaryExpr> futemp36 = std::make_shared<FuBinaryExpr>();
		futemp36->type = this->nIntType;
		futemp36->left = newSymbolReference(offsetField.get());
		futemp36->op = FuToken::plus;
		futemp36->right = newLiteralLong(5);
		std::shared_ptr<FuBinaryExpr> futemp37 = std::make_shared<FuBinaryExpr>();
		futemp37->type = this->boolType;
		futemp37->left = futemp36;
		futemp37->op = FuToken::greaterOrEqual;
		futemp37->right = newSymbolReference(inputLengthField.get());
		...
		switch2->cases[6].body.push_back(futemp38);
		...
		futemp39->type = this->nIntType;
		futemp39->left = newSymbolReference(offsetField.get());
		futemp39->op = FuToken::plus;
		futemp39->right = newLiteralLong(1);
		substringCall->arguments.push_back(futemp39);
		...
		std::shared_ptr<FuPrefixExpr> futemp40 = std::make_shared<FuPrefixExpr>();
		futemp40->type = this->boolType;
		futemp40->op = FuToken::exclamationMark;
		futemp40->inner = tryParseCall;
		std::shared_ptr<FuIf> futemp41 = std::make_shared<FuIf>();
		futemp41->cond = futemp40;
		futemp41->onTrue = newReturnNull();
		...
		futemp42->left = newSymbolReference(offsetField.get());
		futemp42->op = FuToken::addAssign;
		futemp42->right = newLiteralLong(4);
		switch2->cases[6].body.push_back(futemp42);
		std::shared_ptr<FuBreak> futemp43 = std::make_shared<FuBreak>();
		futemp43->loopOrSwitch = switch2.get();
		switch2->cases[6].body.push_back(futemp43);
		switch2->defaultBody.push_back(newReturnNull());
		switch_->cases[2].body.push_back(switch2);
		std::shared_ptr<FuPrefixExpr> futemp44 = std::make_shared<FuPrefixExpr>();
		futemp44->type = this->nIntType;
		futemp44->op = FuToken::increment;
		futemp44->inner = newSymbolReference(offsetField.get());
		std::shared_ptr<FuBinaryExpr> futemp45 = std::make_shared<FuBinaryExpr>();
		futemp45->type = this->nIntType;
		futemp45->left = newSymbolReference(startOffsetVar.get());
		futemp45->op = FuToken::assign;
		futemp45->right = futemp44;
		switch_->cases[2].body.push_back(futemp45);
		std::shared_ptr<FuBreak> futemp46 = std::make_shared<FuBreak>();
		futemp46->loopOrSwitch = switch_.get();
		switch_->cases[2].body.push_back(futemp46);
		std::shared_ptr<FuPostfixExpr> futemp47 = std::make_shared<FuPostfixExpr>();
		futemp47->inner = newSymbolReference(offsetField.get());
		futemp47->op = FuToken::increment;
		switch_->defaultBody.push_back(futemp47);
		std::shared_ptr<FuBreak> futemp48 = std::make_shared<FuBreak>();
		futemp48->loopOrSwitch = switch_.get();
		switch_->defaultBody.push_back(futemp48);
		whileBlock->statements.push_back(switch_);
	...
	futemp23->op = FuToken::greaterOrEqual;
	futemp23->right = futemp22;
	std::shared_ptr<FuBinaryExpr> futemp24 = std::make_shared<FuBinaryExpr>();
	futemp24->type = this->boolType;
	futemp24->left = newOffsetLessInputLength(offsetField.get(), inputLengthField.get());
	futemp24->op = FuToken::condAnd;
	futemp24->right = futemp23;
	std::shared_ptr<FuLiteralChar> futemp25 = std::make_shared<FuLiteralChar>();
	futemp25->type = this->intType;
	futemp25->value = '9';
	...
	futemp26->type = this->boolType;
	futemp26->left = newInputAtOffset(inputField.get(), offsetField.get());
	futemp26->op = FuToken::lessOrEqual;
	futemp26->right = futemp25;
	std::shared_ptr<FuBinaryExpr> futemp27 = std::make_shared<FuBinaryExpr>();
	futemp27->type = this->boolType;
	futemp27->left = futemp24;
	futemp27->op = FuToken::condAnd;
	futemp27->right = futemp26;
	std::shared_ptr<FuReturn> futemp28 = std::make_shared<FuReturn>();
	futemp28->value = futemp27;
	jsonParserSeeDigitMethod->body = futemp28;
	...
	std::shared_ptr<FuMethod> jsonParserParseDigitsMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, this->voidType, FuId::none, "ParseDigits", true);
	{
		...
		while_->body = futemp29;
		methodBlock->statements.push_back(while_);
		jsonParserParseDigitsMethod->body = methodBlock;
	}
	jsonParserClass->add(jsonParserParseDigitsMethod);
	std::shared_ptr<FuDynamicPtrType> futemp29 = std::make_shared<FuDynamicPtrType>();
	futemp29->nullable = true;
	futemp29->class_ = jsonNumberClass.get();
	std::shared_ptr<FuMethod> jsonParserParseNumberMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, futemp29, FuId::none, "ParseNumber", true);
	{
		std::shared_ptr<FuBlock> methodBlock = std::make_shared<FuBlock>();
		std::shared_ptr<FuVar> startOffsetVar = FuVar::new_(this->nIntType, "startOffset", newSymbolReference(offsetField.get()));
		methodBlock->statements.push_back(startOffsetVar);
		methodBlock->add(startOffsetVar);
		std::shared_ptr<FuLiteralChar> futemp30 = std::make_shared<FuLiteralChar>();
		futemp30->type = this->intType;
		futemp30->value = '-';
		std::shared_ptr<FuBinaryExpr> futemp31 = std::make_shared<FuBinaryExpr>();
		futemp31->type = this->boolType;
		futemp31->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp31->op = FuToken::equal;
		futemp31->right = futemp30;
		std::shared_ptr<FuPostfixExpr> futemp32 = std::make_shared<FuPostfixExpr>();
		futemp32->inner = newSymbolReference(offsetField.get());
		futemp32->op = FuToken::increment;
		std::shared_ptr<FuIf> futemp33 = std::make_shared<FuIf>();
		futemp33->cond = futemp31;
		futemp33->onTrue = futemp32;
		...
		std::shared_ptr<FuBinaryExpr> futemp35 = std::make_shared<FuBinaryExpr>();
		futemp35->type = this->intType;
		futemp35->left = newSymbolReference(inputField.get());
		futemp35->op = FuToken::leftBracket;
		futemp35->right = futemp34;
		std::shared_ptr<FuLiteralChar> futemp36 = std::make_shared<FuLiteralChar>();
		futemp36->type = this->intType;
		futemp36->value = '0';
		std::shared_ptr<FuBinaryExpr> futemp37 = std::make_shared<FuBinaryExpr>();
		futemp37->type = this->boolType;
		futemp37->left = futemp35;
		futemp37->op = FuToken::greater;
		futemp37->right = futemp36;
		std::shared_ptr<FuIf> futemp38 = std::make_shared<FuIf>();
		futemp38->cond = futemp37;
		futemp38->onTrue = newCall(jsonParserParseDigitsMethod.get());
		methodBlock->statements.push_back(futemp38);
		std::shared_ptr<FuBlock> ifBlock = std::make_shared<FuBlock>();
		std::shared_ptr<FuLiteralChar> futemp39 = std::make_shared<FuLiteralChar>();
		futemp39->type = this->intType;
		futemp39->value = '.';
		std::shared_ptr<FuBinaryExpr> futemp40 = std::make_shared<FuBinaryExpr>();
		futemp40->type = this->boolType;
		futemp40->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp40->op = FuToken::equal;
		futemp40->right = futemp39;
		std::shared_ptr<FuBinaryExpr> futemp41 = std::make_shared<FuBinaryExpr>();
		futemp41->type = this->boolType;
		futemp41->left = newOffsetLessInputLength(offsetField.get(), inputLengthField.get());
		futemp41->op = FuToken::condAnd;
		futemp41->right = futemp40;
		std::shared_ptr<FuIf> futemp42 = std::make_shared<FuIf>();
		futemp42->cond = futemp41;
		futemp42->onTrue = ifBlock;
		futemp42->onFalse = nullptr;
		methodBlock->statements.push_back(futemp42);
		std::shared_ptr<FuPostfixExpr> futemp43 = std::make_shared<FuPostfixExpr>();
		futemp43->inner = newSymbolReference(offsetField.get());
		futemp43->op = FuToken::increment;
		ifBlock->statements.push_back(futemp43);
		...
		futemp44->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp44->op = FuToken::or_;
		futemp44->right = newLiteralLong(32);
		std::shared_ptr<FuLiteralChar> futemp45 = std::make_shared<FuLiteralChar>();
		futemp45->type = this->intType;
		futemp45->value = 'e';
		std::shared_ptr<FuBinaryExpr> futemp46 = std::make_shared<FuBinaryExpr>();
		futemp46->type = this->boolType;
		futemp46->left = futemp44;
		futemp46->op = FuToken::equal;
		futemp46->right = futemp45;
		std::shared_ptr<FuBinaryExpr> futemp47 = std::make_shared<FuBinaryExpr>();
		futemp47->type = this->boolType;
		futemp47->left = newOffsetLessInputLength(offsetField.get(), inputLengthField.get());
		futemp47->op = FuToken::condAnd;
		futemp47->right = futemp46;
		std::shared_ptr<FuIf> futemp48 = std::make_shared<FuIf>();
		futemp48->cond = futemp47;
		futemp48->onTrue = if2Block;
		futemp48->onFalse = nullptr;
		methodBlock->statements.push_back(futemp48);
		std::shared_ptr<FuPrefixExpr> futemp49 = std::make_shared<FuPrefixExpr>();
		futemp49->type = this->nIntType;
		futemp49->op = FuToken::increment;
		futemp49->inner = newSymbolReference(offsetField.get());
		std::shared_ptr<FuBinaryExpr> futemp50 = std::make_shared<FuBinaryExpr>();
		futemp50->type = this->boolType;
		futemp50->left = futemp49;
		futemp50->op = FuToken::less;
		futemp50->right = newSymbolReference(inputLengthField.get());
		std::shared_ptr<FuLiteralChar> futemp51 = std::make_shared<FuLiteralChar>();
		futemp51->type = this->intType;
		futemp51->value = '+';
		std::shared_ptr<FuBinaryExpr> futemp52 = std::make_shared<FuBinaryExpr>();
		futemp52->type = this->boolType;
		futemp52->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp52->op = FuToken::equal;
		futemp52->right = futemp51;
		std::shared_ptr<FuLiteralChar> futemp53 = std::make_shared<FuLiteralChar>();
		futemp53->type = this->intType;
		futemp53->value = '-';
		std::shared_ptr<FuBinaryExpr> futemp54 = std::make_shared<FuBinaryExpr>();
		futemp54->type = this->boolType;
		futemp54->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp54->op = FuToken::equal;
		futemp54->right = futemp53;
		std::shared_ptr<FuBinaryExpr> futemp55 = std::make_shared<FuBinaryExpr>();
		futemp55->type = this->boolType;
		futemp55->left = futemp52;
		futemp55->op = FuToken::condOr;
		futemp55->right = futemp54;
		std::shared_ptr<FuBinaryExpr> futemp56 = std::make_shared<FuBinaryExpr>();
		futemp56->type = this->boolType;
		futemp56->left = futemp50;
		futemp56->op = FuToken::condAnd;
		futemp56->right = futemp55;
		std::shared_ptr<FuPostfixExpr> futemp57 = std::make_shared<FuPostfixExpr>();
		futemp57->inner = newSymbolReference(offsetField.get());
		futemp57->op = FuToken::increment;
		std::shared_ptr<FuIf> futemp58 = std::make_shared<FuIf>();
		futemp58->cond = futemp56;
		futemp58->onTrue = futemp57;
		futemp58->onFalse = nullptr;
		if2Block->statements.push_back(futemp58);
		...
		std::shared_ptr<FuBinaryExpr> futemp59 = std::make_shared<FuBinaryExpr>();
		futemp59->type = this->nIntType;
		futemp59->left = newSymbolReference(offsetField.get());
		futemp59->op = FuToken::minus;
		futemp59->right = newSymbolReference(startOffsetVar.get());
		substringCall->arguments.push_back(futemp59);
		std::shared_ptr<FuCallExpr> tryParseCall = newMethodCall(dVar.get(), doubleTryParseMethod.get());
		tryParseCall->arguments.push_back(substringCall);
		std::shared_ptr<FuPrefixExpr> futemp60 = std::make_shared<FuPrefixExpr>();
		futemp60->type = this->boolType;
		futemp60->op = FuToken::exclamationMark;
		futemp60->inner = tryParseCall;
		std::shared_ptr<FuIf> futemp61 = std::make_shared<FuIf>();
		futemp61->cond = futemp60;
		futemp61->onTrue = newReturnNull();
		futemp61->onFalse = nullptr;
		methodBlock->statements.push_back(futemp61);
		std::shared_ptr<FuAggregateInitializer> init = std::make_shared<FuAggregateInitializer>();
		std::shared_ptr<FuBinaryExpr> futemp62 = std::make_shared<FuBinaryExpr>();
		futemp62->left = newSymbolReference(jsonNumberValueField.get());
		futemp62->op = FuToken::assign;
		futemp62->right = newSymbolReference(dVar.get());
		init->items.push_back(futemp62);
		std::shared_ptr<FuDynamicPtrType> futemp63 = std::make_shared<FuDynamicPtrType>();
		futemp63->class_ = jsonNumberClass.get();
		std::shared_ptr<FuPrefixExpr> futemp64 = std::make_shared<FuPrefixExpr>();
		futemp64->type = futemp63;
		futemp64->op = FuToken::new_;
		futemp64->inner = init;
		std::shared_ptr<FuReturn> futemp65 = std::make_shared<FuReturn>();
		futemp65->value = futemp64;
		methodBlock->statements.push_back(futemp65);
	...
	std::shared_ptr<FuMethod> jsonParserParseKeywordMethod = FuMethod::new_(jsonParserClass.get(), FuVisibility::private_, FuCallType::normal, this->boolType, FuId::none, "ParseKeyword", true, sParameter);
	{
		...
		std::shared_ptr<FuVar> cVar = FuVar::new_(this->intType, "c");
		loop->add(cVar);
		std::shared_ptr<FuPrefixExpr> futemp30 = std::make_shared<FuPrefixExpr>();
		futemp30->type = this->nIntType;
		futemp30->op = FuToken::increment;
		futemp30->inner = newSymbolReference(offsetField.get());
		std::shared_ptr<FuBinaryExpr> futemp31 = std::make_shared<FuBinaryExpr>();
		futemp31->type = this->boolType;
		futemp31->left = futemp30;
		futemp31->op = FuToken::greaterOrEqual;
		futemp31->right = newSymbolReference(inputLengthField.get());
		std::shared_ptr<FuBinaryExpr> futemp32 = std::make_shared<FuBinaryExpr>();
		futemp32->type = this->boolType;
		futemp32->left = newInputAtOffset(inputField.get(), offsetField.get());
		futemp32->op = FuToken::notEqual;
		futemp32->right = newSymbolReference(cVar.get());
		std::shared_ptr<FuBinaryExpr> futemp33 = std::make_shared<FuBinaryExpr>();
		futemp33->type = this->boolType;
		futemp33->left = futemp31;
		futemp33->op = FuToken::condOr;
		futemp33->right = futemp32;
		std::shared_ptr<FuIf> futemp34 = std::make_shared<FuIf>();
		futemp34->cond = futemp33;
		futemp34->onTrue = newReturnFalse();
		futemp34->onFalse = nullptr;
		foreachBlock->statements.push_back(futemp34);
		methodBlock->statements.push_back(loop);
		std::shared_ptr<FuPostfixExpr> futemp35 = std::make_shared<FuPostfixExpr>();
		futemp35->inner = newSymbolReference(offsetField.get());
		futemp35->op = FuToken::increment;
		methodBlock->statements.push_back(futemp35);
	...
	std::shared_ptr<FuLiteralNull> futemp30 = std::make_shared<FuLiteralNull>();
	futemp30->type = this->nullType;
	std::shared_ptr<FuSelectExpr> futemp31 = std::make_shared<FuSelectExpr>();
	futemp31->cond = newCall(jsonParserSkipWhitespaceMethod.get());
	futemp31->onTrue = newCall(jsonParserParseElementMethod.get());
	futemp31->onFalse = futemp30;
	std::shared_ptr<FuReturn> futemp32 = std::make_shared<FuReturn>();
	futemp32->value = futemp31;
	jsonParserParseWhitespaceAndElementMethod->body = futemp32;
	jsonParserClass->add(jsonParserParseWhitespaceAndElementMethod);
	{
		...
		std::shared_ptr<FuBlock> methodBlock = std::make_shared<FuBlock>();
		std::shared_ptr<FuBinaryExpr> futemp33 = std::make_shared<FuBinaryExpr>();
		futemp33->type = this->stringPtrType;
		futemp33->left = newSymbolReference(inputField.get());
		futemp33->op = FuToken::assign;
		futemp33->right = newSymbolReference(sParam.get());
		methodBlock->statements.push_back(futemp33);
		std::shared_ptr<FuBinaryExpr> futemp34 = std::make_shared<FuBinaryExpr>();
		futemp34->type = this->nIntType;
		futemp34->left = newSymbolReference(offsetField.get());
		futemp34->op = FuToken::assign;
		futemp34->right = newLiteralLong(0);
		methodBlock->statements.push_back(futemp34);
		std::shared_ptr<FuBinaryExpr> futemp35 = std::make_shared<FuBinaryExpr>();
		futemp35->type = this->nIntType;
		futemp35->left = newSymbolReference(inputLengthField.get());
		futemp35->op = FuToken::assign;
		futemp35->right = newMemberReference(newSymbolReference(sParam.get()), stringLengthProperty.get());
		methodBlock->statements.push_back(futemp35);
		std::shared_ptr<FuDynamicPtrType> futemp36 = std::make_shared<FuDynamicPtrType>();
		futemp36->nullable = true;
		futemp36->class_ = jsonElementClass.get();
		std::shared_ptr<FuVar> resultVar = FuVar::new_(futemp36, "result", newCall(jsonParserParseWhitespaceAndElementMethod.get()));
		methodBlock->statements.push_back(resultVar);
		std::shared_ptr<FuLiteralNull> futemp37 = std::make_shared<FuLiteralNull>();
		futemp37->type = this->nullType;
		std::shared_ptr<FuSelectExpr> futemp38 = std::make_shared<FuSelectExpr>();
		futemp38->cond = newCall(jsonParserSkipWhitespaceMethod.get());
		futemp38->onTrue = futemp37;
		futemp38->onFalse = newSymbolReference(resultVar.get());
		std::shared_ptr<FuReturn> futemp39 = std::make_shared<FuReturn>();
		futemp39->value = futemp38;
		methodBlock->statements.push_back(futemp39);
...
void FuConsoleHost::usage(std::string_view app)
{
	...
	for (;;) {
		...
		if (const FuSymbolReference *symbol = dynamic_cast<const FuSymbolReference *>(expr)) {
			if (FuVar *def = dynamic_cast<FuVar *>(symbol->symbol)) {
				def->isAssigned = true;
				{
					FuScope * fuSwitchValue = symbol->symbol->parent;
					if (FuFor *forLoop = dynamic_cast<FuFor *>(fuSwitchValue))
						forLoop->isRange = false;
					else if (dynamic_cast<const FuForeach *>(fuSwitchValue))
				...
				{
					std::shared_ptr<FuType> fuSwitchValue = symbol->left->type;
					if (dynamic_cast<const FuStorageType *>(fuSwitchValue.get()))
						expr = symbol->left.get();
					else if (dynamic_cast<const FuReadWriteClassType *>(fuSwitchValue.get()))
						return;
					else if (dynamic_cast<const FuClassType *>(fuSwitchValue.get())) {
			...
			{
				std::shared_ptr<FuType> fuSwitchValue = indexing->left->type;
				if (dynamic_cast<const FuStorageType *>(fuSwitchValue.get()))
...
{
	{
		std::shared_ptr<FuExpr> fuSwitchValue = visitExpr(value, true);
		const FuSymbolReference * symbol;
		const FuClass * klass;
		if (dynamic_cast<const FuLiteralNull *>(fuSwitchValue.get())) {
		}
		else if ((symbol = dynamic_cast<const FuSymbolReference *>(fuSwitchValue.get())) && (klass = dynamic_cast<const FuClass *>(symbol->symbol)))
			checkIsHierarchy(switchPtr, statement->value.get(), klass, value.get(), "case", "always match", "never match");
		else if (const FuVar *def = dynamic_cast<const FuVar *>(fuSwitchValue.get()))
...
void FuSema::visitSwitch(FuSwitch * statement)
{
	...
	if (statement->value != this->poison && statement->value->type != this->poison) {
		{
			std::shared_ptr<FuType> fuSwitchValue = statement->value->type;
			const FuIntegerType * i;
			const FuClassType * klass;
			if ((i = dynamic_cast<const FuIntegerType *>(fuSwitchValue.get())) && i->id != FuId::longType) {
			}
			else if (dynamic_cast<const FuEnum *>(fuSwitchValue.get())) {
			}
			else if ((klass = dynamic_cast<const FuClassType *>(fuSwitchValue.get())) && !dynamic_cast<const FuStorageType *>(klass)) {
			}
			else
...
void GenBase::writeSwitchCaseCond(bool switchVar, const FuExpr * switchValue, const FuExpr * value, FuPriority parent)
{
	const FuBinaryExpr * when1;
	if ((when1 = dynamic_cast<const FuBinaryExpr *>(value)) && when1->op == FuToken::when) {
		if (parent > FuPriority::selectCond)
			writeChar('(');
		writeSwitchCaseCond(switchVar, switchValue, when1->left.get(), FuPriority::condAnd);
		write(" && ");
...
{
	bool switchVar = needsSwitchVar(statement->value.get());
	if (switchVar) {
		openBlock();
		writeSwitchVar(statement->value.get());
		writeCharLine(';');
	}
	for (const FuCase &kase : statement->cases) {
		for (const std::shared_ptr<FuExpr> &value : kase.values) {
	...
	for (const FuCase &kase : statement->cases) {
		...
		for (const std::shared_ptr<FuExpr> &value : kase.values) {
			write(op);
			writeSwitchCaseCond(switchVar, statement->value.get(), value.get(), parent);
			op = " || ";
		}
		writeChar(')');
...
void GenC::writeCamelCaseNotKeyword(std::string_view name)
{
	if (name == "this")
		write("self");
	else if (name == "Asm" || name == "Assert" || name == "Auto" || name == "Bool" || name == "Break" || name == "Byte" || name == "Case" || name == "Char" || name == "Class" || name == "Const" || name == "Continue" || name == "Default" || name == "Do" || name == "Double" || name == "Else" || name == "Enum" || name == "Extern" || name == "False" || name == "Float" || name == "For" || name == "Foreach" || name == "Goto" || name == "If" || name == "Inline" || name == "Int" || name == "Long" || name == "Nullptr" || name == "Register" || name == "Restrict" || name == "Return" || name == "Short" || name == "Signed" || name == "Sizeof" || name == "Static" || name == "Struct" || name == "Switch" || name == "True" || name == "Typedef" || name == "Typeof" || name == "Union" || name == "Unsigned" || name == "Void" || name == "Volatile" || name == "While" || name == "asm" || name == "auto" || name == "char" || name == "extern" || name == "goto" || name == "inline" || name == "nullptr" || name == "register" || name == "restrict" || name == "signed" || name == "sizeof" || name == "struct" || name == "typedef" || name == "typeof" || name == "union" || name == "unsigned" || name == "volatile") {
		writeCamelCase(name);
		writeChar('_');
	}
	else
...
void GenC::writeLocalName(const FuSymbol * symbol, FuPriority parent)
{
	if (const FuForeach *forEach = dynamic_cast<const FuForeach *>(symbol->parent)) {
		...
		{
			FuId fuSwitchValue = klass->class_->id;
			const FuClassType * elementClass;
			if (fuSwitchValue == FuId::stringClass || (fuSwitchValue == FuId::listClass && (!dynamic_cast<const FuStorageType *>(klass->getElementType().get()) || ((elementClass = dynamic_cast<const FuClassType *>(klass->getElementType().get())) && elementClass->class_->id == FuId::listClass)))) {
			...
			else if (fuSwitchValue == FuId::arrayStorageClass) {
...
void GenC::writePrintfNotInterpolated(const std::vector<std::shared_ptr<FuExpr>> * args, bool newLine)
{
	write("\"%");
	{
		std::shared_ptr<FuType> fuSwitchValue = (*args)[0]->type;
		if (const FuIntegerType *intType = dynamic_cast<const FuIntegerType *>(fuSwitchValue.get())) {
			writePrintfIntPrefix(intType->id);
			writeChar('d');
		}
		else if (dynamic_cast<const FuFloatingType *>(fuSwitchValue.get()))
			writeChar('g');
		else
			writeChar('s');
	}
...
{
	writeCamelCase(name);
	if (name == "And" || name == "Asm" || name == "Auto" || name == "Bool" || name == "Break" || name == "Byte" || name == "Case" || name == "Catch" || name == "Char" || name == "Class" || name == "Const" || name == "Continue" || name == "Default" || name == "Delete" || name == "Do" || name == "Double" || name == "Else" || name == "Enum" || name == "Explicit" || name == "Export" || name == "Extern" || name == "False" || name == "Float" || name == "For" || name == "Friend" || name == "Goto" || name == "If" || name == "Inline" || name == "Int" || name == "Long" || name == "Namespace" || name == "New" || name == "Not" || name == "Nullptr" || name == "Operator" || name == "Or" || name == "Override" || name == "Private" || name == "Protected" || name == "Public" || name == "Register" || name == "Return" || name == "Short" || name == "Signed" || name == "Sizeof" || name == "Static" || name == "Struct" || name == "Switch" || name == "Throw" || name == "True" || name == "Try" || name == "Typedef" || name == "Union" || name == "Unsigned" || name == "Using" || name == "Virtual" || name == "Void" || name == "Volatile" || name == "While" || name == "Xor" || name == "and" || name == "asm" || name == "auto" || name == "catch" || name == "char" || name == "delete" || name == "explicit" || name == "export" || name == "extern" || name == "friend" || name == "goto" || name == "inline" || name == "namespace" || name == "not" || name == "nullptr" || name == "operator" || name == "or" || name == "private" || name == "register" || name == "signed" || name == "sizeof" || name == "struct" || name == "try" || name == "typedef" || name == "union" || name == "unsigned" || name == "using" || name == "volatile" || name == "xor")
		writeChar('_');
}
...
void GenCpp::writeCollectionType(const FuClassType * klass)
{
	...
	case FuId::priorityQueueClass:
		...
		writeType(elementType, false);
		write(", ");
		writeType(klass->typeArg1.get(), false);
		write(">>");
		return;
	case FuId::hashSetClass:
...
void GenCpp::writePop(const FuExpr * obj, FuPriority parent, int p, std::string_view front, std::string_view frontSuffix)
{
	if (parent == FuPriority::statement) {
		startMethodCall(obj);
		write("pop()");
...
void GenCpp::writeCallExpr(const FuType * type, const FuExpr * obj, const FuMethod * method, const std::vector<std::shared_ptr<FuExpr>> * args, FuPriority parent)
{
	...
	case FuId::priorityQueueEnqueue:
		writeMethodCall(obj, "emplace", (*args)[0].get(), (*args)[1].get());
		break;
...
void GenCpp::visitBinaryExpr(const FuBinaryExpr * expr, FuPriority parent)
{
	...
	case FuToken::is:
		if (const FuSymbolReference *symbol = dynamic_cast<const FuSymbolReference *>(expr->right.get())) {
			...
			writeIsVar(false, expr->left.get(), def, parent);
			return;
		}
		else
			std::abort();
...
void GenCpp::writeSwitchVar(const FuExpr * expr)
{
	if (expr->type->id == FuId::stringStorageType) {
...
void GenCpp::writeSwitchCaseCond(bool switchVar, const FuExpr * switchValue, const FuExpr * value, FuPriority parent)
{
	...
	if ((symbol = dynamic_cast<const FuSymbolReference *>(value)) && dynamic_cast<const FuClass *>(symbol->symbol))
		...
		if (parent == FuPriority::argument)
			writeType(def->type.get(), true);
		writeIsVar(switchVar, switchValue, def, parent);
	}
	else
		GenBase::writeSwitchCaseCond(switchVar, switchValue, value, parent);
}
...
void GenCs::writeCallExpr(const FuType * type, const FuExpr * obj, const FuMethod * method, const std::vector<std::shared_ptr<FuExpr>> * args, FuPriority parent)
{
	...
	case FuId::uTF8GetBytesAlloc:
		include("System.Text");
		writeCall("Encoding.UTF8.GetBytes", (*args)[0].get());
		break;
	case FuId::uTF8GetBytesCopy:
		include("System.Text");
...
void GenD::writeCallExpr(const FuType * type, const FuExpr * obj, const FuMethod * method, const std::vector<std::shared_ptr<FuExpr>> * args, FuPriority parent)
{
	...
	case FuId::uTF8GetBytesAlloc:
		include("std.string");
		writePostfix((*args)[0].get(), ".representation");
		break;
	case FuId::uTF8GetBytesCopy:
		include("std.string");
		include("std.algorithm");
...
void GenD::writeIsVar(bool switchVar, const FuExpr * left, const FuExpr * right, FuPriority parent)
{
	if (parent > FuPriority::equality)
		writeChar('(');
	const FuSymbolReference * symbol;
	...
	if ((symbol = dynamic_cast<const FuSymbolReference *>(right)) && (klass = dynamic_cast<const FuClass *>(symbol->symbol))) {
		...
		write(klass->name);
		write(") ");
		writeExprOrSwitchValue(switchVar, left, FuPriority::primary);
	}
	else if (const FuVar *def = dynamic_cast<const FuVar *>(right)) {
		writeChar('(');
		writeName(def);
...
void GenD::visitBinaryExpr(const FuBinaryExpr * expr, FuPriority parent)
{
	switch (expr->op) {
	case FuToken::is:
		writeIsVar(false, expr->left.get(), expr->right.get(), parent >= FuPriority::or_ && parent <= FuPriority::mul ? FuPriority::primary : parent);
		return;
	case FuToken::plus:
		if (expr->type->id == FuId::stringStorageType) {
			expr->left->accept(this, FuPriority::assign);
...
void GenD::writeSwitchCaseCond(bool switchVar, const FuExpr * switchValue, const FuExpr * value, FuPriority parent)
{
	const FuSymbolReference * symbol;
	if ((symbol = dynamic_cast<const FuSymbolReference *>(value)) && dynamic_cast<const FuClass *>(symbol->symbol))
		writeIsVar(switchVar, switchValue, value, parent);
...
void GenJava::writeJavaType(const FuType * type, bool promote, bool needClass)
{
	if (dynamic_cast<const FuNumericType *>(type)) {
		...
		case FuId::priorityQueueClass:
			...
			writeJavaType(klass->typeArg0.get(), false, true);
			write(", ");
			writeJavaType(klass->typeArg1.get(), false, true);
			write(">>");
			break;
		case FuId::hashSetClass:
...
void GenJava::writeEqual(const FuExpr * left, const FuExpr * right, FuPriority parent, bool not_)
{
	if ((left == nullptr || dynamic_cast<const FuStringType *>(left->type.get())) && dynamic_cast<const FuStringType *>(right->type.get())) {
		...
		if (isUnsignedByteIndexing(left) && (rightLiteral = dynamic_cast<const FuLiteralLong *>(right)) && rightLiteral->type->id == FuId::byteRange) {
			...
			if (dynamic_cast<const FuNumericType *>(left->type.get()) && isCollectionIndexing(left) && isCollectionIndexing(right))
				writeStaticCastType(left->type.get());
			GenBase::writeEqual(left, right, parent, not_);
		}
	}
}
...
void GenJava::writeCallExpr(const FuType * type, const FuExpr * obj, const FuMethod * method, const std::vector<std::shared_ptr<FuExpr>> * args, FuPriority parent)
{
	...
	case FuId::uTF8GetBytesAlloc:
		include("java.nio.charset.StandardCharsets");
		writePostfix((*args)[0].get(), ".getBytes(StandardCharsets.UTF_8)");
		break;
	case FuId::uTF8GetBytesCopy:
		include("java.nio.ByteBuffer");
...
void GenJava::writePriorityQueue()
{
...
void GenJsNoModule::writeCallExpr(const FuType * type, const FuExpr * obj, const FuMethod * method, const std::vector<std::shared_ptr<FuExpr>> * args, FuPriority parent)
{
	...
	case FuId::uTF8GetByteCount:
		...
		break;
	case FuId::uTF8GetBytesAlloc:
		writeCall("new TextEncoder().encode", (*args)[0].get());
		break;
	case FuId::uTF8GetBytesCopy:
...
void GenJsNoModule::writeIsVar(bool switchVar, const FuExpr * expr, std::string_view name, const FuSymbol * klass, FuPriority parent)
{
	if (parent > FuPriority::rel)
		writeChar('(');
	if (name.data() != nullptr) {
		...
		writeExprOrSwitchValue(switchVar, expr, FuPriority::rel);
	write(" instanceof ");
	write(klass->name);
	if (parent > FuPriority::rel)
		writeChar(')');
...
void GenJsNoModule::visitBinaryExpr(const FuBinaryExpr * expr, FuPriority parent)
{
	...
	if (expr->op == FuToken::slash && dynamic_cast<const FuIntegerType *>(expr->type.get()) && expr->type->id != FuId::longType) {
		...
		writeIsVar(false, expr->left.get(), def->name, def->type.get(), parent);
	else
		GenBase::visitBinaryExpr(expr, parent);
}
...
void GenJsNoModule::visitForeach(const FuForeach * statement)
{
	...
	case FuId::sortedSetClass:
		...
		write(" of ");
		{
			std::shared_ptr<FuType> fuSwitchValue = klass->getElementType();
			if (const FuNumericType *number = dynamic_cast<const FuNumericType *>(fuSwitchValue.get())) {
				write("new ");
				writeArrayElementType(number);
				write("Array(");
			}
	...
	case FuId::orderedDictionaryClass:
		...
		if (klass->class_->id == FuId::orderedDictionaryClass)
			...
			{
				std::shared_ptr<FuType> fuSwitchValue = statement->getVar()->type;
				if (dynamic_cast<const FuStringType *>(fuSwitchValue.get())) {
					if (klass->class_->id == FuId::sortedDictionaryClass)
						write(".sort((a, b) => a[0].localeCompare(b[0]))");
				}
				else if (dynamic_cast<const FuNumericType *>(fuSwitchValue.get()) || dynamic_cast<const FuEnum *>(fuSwitchValue.get())) {
					write(".map(e => [+e[0], e[1]])");
					if (klass->class_->id == FuId::sortedDictionaryClass)
...
void GenJsNoModule::writeSwitchVar(const FuExpr * expr)
{
	write("const fuSwitchValue = ");
	expr->accept(this, FuPriority::argument);
}

void GenJsNoModule::writeSwitchCaseCond(bool switchVar, const FuExpr * switchValue, const FuExpr * value, FuPriority parent)
{
	const FuSymbolReference * symbol;
	if ((symbol = dynamic_cast<const FuSymbolReference *>(value)) && dynamic_cast<const FuClass *>(symbol->symbol))
		writeIsVar(switchVar, switchValue, std::string_view(), symbol->symbol, parent);
	else if (const FuVar *def = dynamic_cast<const FuVar *>(value))
		writeIsVar(switchVar, switchValue, parent == FuPriority::condAnd ? def->name : std::string_view(), def->type.get(), parent);
	else
		GenBase::writeSwitchCaseCond(switchVar, switchValue, value, parent);
}
...
void GenTs::writeType(const FuType * type, bool readOnly)
{
	if (dynamic_cast<const FuNumericType *>(type))
		...
		{
			FuId fuSwitchValue = klass->class_->id;
			if (fuSwitchValue == FuId::stringClass)
				write("string");
			else if ((fuSwitchValue == FuId::arrayPtrClass && !dynamic_cast<const FuNumericType *>(klass->getElementType().get())) || (fuSwitchValue == FuId::arrayStorageClass && !dynamic_cast<const FuNumericType *>(klass->getElementType().get())) || fuSwitchValue == FuId::listClass || fuSwitchValue == FuId::queueClass || fuSwitchValue == FuId::stackClass) {
				if (readOnly)
					write("readonly ");
...
bool GenSwift::needsVarBytes(const std::vector<std::shared_ptr<FuStatement>> * statements)
{
	...
	for (const std::shared_ptr<FuStatement> &statement : *statements) {
		const FuCallExpr * call;
		if ((call = dynamic_cast<const FuCallExpr *>(statement.get())) && call->method->symbol->id == FuId::uTF8GetBytesCopy) {
			if (++count == 2)
				return true;
		}
	}
...
void GenPy::writeTypeAnnotation(const FuType * type, bool nullable)
{
	if (dynamic_cast<const FuIntegerType *>(type))
		...
		case FuId::priorityQueueClass:
			write("list[tuple[");
			writeTypeAnnotation(klass->typeArg1.get());
			write(", ");
			writeTypeAnnotation(klass->typeArg0.get());
			write("]]");
			break;
		case FuId::hashSetClass:
		case FuId::sortedSetClass:
