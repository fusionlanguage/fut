...
namespace Fusion
{
	...
	public abstract class FuParserHost
	{
		...
		internal FuProgram Program = null!;
	...
	public abstract class FuLexer
	{
		...
		protected FuParserHost Host = null!;
	...
	public class FuSelectExpr : FuExpr
	{
		...
		internal FuExpr Cond = null!;

		internal FuExpr OnTrue = null!;

		internal FuExpr OnFalse = null!;
	...
	public abstract class FuLoop : FuCondCompletionStatement
	{
		...
		internal FuStatement Body = null!;
	...
	class FuBreak : FuStatement
	{
		...
		internal FuCondCompletionStatement LoopOrSwitch = null!;
	...
	class FuForeach : FuLoop
	{
		...
		internal FuExpr Collection = null!;
	...
	class FuIf : FuCondCompletionStatement
	{
		...
		internal FuStatement OnTrue = null!;
	...
	public class FuThrow : FuStatement
	{
		...
		internal FuSymbolReference Class = null!;
	...
	public class FuArrayStorageType : FuStorageType
	{
		...
		internal FuExpr LengthExpr = null!;
	...
	public class FuSystem : FuScope
	{
		internal FuSystem()
		{
			...
			priorityQueueClass.AddMethod(this.VoidType, FuId.PriorityQueueEnqueue, "Enqueue", true, FuVar.New(this.TypeParam0, "element"), FuVar.New(this.TypeParam1, "priority"));
		...
		internal FuEnum RegexOptionsEnum = null!;
	...
	public abstract class FuConsoleHost : GenHost
	{
		...
		{
			Console.WriteLine($"Usage: {app} [OPTIONS] -o FILE INPUT.fu");
			Console.WriteLine("Options:");
			Console.WriteLine("-o FILE    Write to the specified file, extension determines language");
			...
			Console.WriteLine("-r FILE.fu Read the specified source file but don't emit code for it");
			Console.WriteLine("-I DIR     Add directory to resource search path");
			Console.WriteLine("-l LANG    Set target language explicitly:");
			Console.WriteLine("           c, cpp, cs, d, java, js, py, swift, ts, d.ts, cl");
	...
	public class FuSema
	{
		...
		bool CoercePermanent(FuExpr expr, FuType type)
		{
			...
			if (expr is FuCallExpr call && call.Method.Symbol!.Id != FuId.ListLast && expr.Type is FuStorageType && !(type is FuStorageType)) {
				ReportError(expr, $"Returned object must be assigned to '{expr.Type}'");
				return false;
			}
			return true;
		}
			...
			if (expr.Value != null) {
				if (type is FuStorageType storage && expr.Value is FuAggregateInitializer init)
					...
					if (type is FuStorageType) {
						...
						else if (expr.Value is FuLiteralNull) {
							this.CurrentScope!.Add(expr);
							return;
						}
						else if (expr.Value is FuSymbolReference)
	...
	public class GenC : GenCCpp
	{
		...
		protected FuClass CurrentClass = null!;
	...
	public class GenCpp : GenCCpp
	{
		...
		internal override void VisitForeach(FuForeach statement)
		{
			...
			if (collectionType.Class.Id == FuId.StringClass) {
				...
				else if (collectionType.GetElementType() is FuOwningType owning) {
					if (collectionType.GetElementType() is FuDynamicPtrType || !(element.Type is FuReadWriteClassType))
						Write("const ");
		...
		protected override void WriteSwitchCaseCond(bool switchVar, FuExpr switchValue, FuExpr value, FuPriority parent)
		{
			switch (value) {
			...
			case FuVar def:
				...
				break;
			default:
				base.WriteSwitchCaseCond(switchVar, switchValue, value, parent);
				break;
			}
		}
	...
	public class GenCs : GenTyped
	{
		...
		protected override void WriteIndexingInfix(FuExpr collection)
		{
		...
		protected override void WriteField(FuField field)
		{
			...
			if (field.Type!.IsFinal() && !field.IsAssignableStorage())
				Write("readonly ");
			WriteVar(field);
			if (!field.Type!.Nullable && field.Type is FuClassType && (field.Type!.IsFinal() ? field.Value is FuLiteralNull && !(field.Type is FuArrayStorageType) : field.Value == null))
				Write(" = null!");
			WriteCharLine(';');
		}
	...
	public class GenD : GenCCppD
	{
		...
		void WriteIsVar(bool switchVar, FuExpr left, FuExpr right, FuPriority parent)
		{
			if (parent > FuPriority.Equality)
				WriteChar('(');
			switch (right) {
		...
		internal override void VisitBinaryExpr(FuBinaryExpr expr, FuPriority parent)
		{
			switch (expr.Op) {
			case FuToken.Is:
				WriteIsVar(false, expr.Left, expr.Right!, parent >= FuPriority.Or && parent <= FuPriority.Mul ? FuPriority.Primary : parent);
				return;
			case FuToken.Plus:
				if (expr.Type!.Id == FuId.StringStorageType) {
					expr.Left.Accept(this, FuPriority.Assign);
	...
	public class GenJava : GenTyped
	{
		...
		protected override void WriteEqual(FuExpr? left, FuExpr right, FuPriority parent, bool not)
		{
			if ((left == null || left!.Type is FuStringType) && right.Type is FuStringType) {
				if (not)
					WriteChar('!');
				WriteExprOrSwitchValue(left == null, left!, FuPriority.Primary);
		...
		void WriteMethodCallNoArgs(FuExpr obj, string method)
		{
			obj.Accept(this, FuPriority.Primary);
			WriteChar('.');
			Write(method);
		...
		void WriteCollectionGet(FuExpr obj, string method, FuPriority parent)
		{
			if (IsUnsignedByte(obj.Type!.AsClassType().GetElementType())) {
				if (parent > FuPriority.And)
					WriteChar('(');
	...
	public class GenJsNoModule : GenBase
	{
		...
		void WriteIsVar(bool switchVar, FuExpr expr, string? name, FuSymbol klass, FuPriority parent)
		{
			if (parent > FuPriority.Rel)
				WriteChar('(');
			if (name != null) {
		...
		internal override void VisitBinaryExpr(FuBinaryExpr expr, FuPriority parent)
		{
			...
			case FuToken.Is when expr.Right is FuVar def:
				WriteIsVar(false, expr.Left, def.Name, def.Type!, parent);
				break;
			default:
				base.VisitBinaryExpr(expr, parent);
				break;
		...
		protected override void WriteSwitchCaseCond(bool switchVar, FuExpr switchValue, FuExpr value, FuPriority parent)
		{
			switch (value) {
			case FuSymbolReference symbol when symbol.Symbol is FuClass:
				WriteIsVar(switchVar, switchValue, null, symbol.Symbol!, parent);
				break;
			case FuVar def:
				WriteIsVar(switchVar, switchValue, parent == FuPriority.CondAnd ? def.Name : null, def.Type!, parent);
				break;
			default:
				base.WriteSwitchCaseCond(switchVar, switchValue, value, parent);
				break;
	...
	public class GenSwift : GenPySwift
	{
		...
		void WriteUnicodeScalar(FuExpr expr)
		{
